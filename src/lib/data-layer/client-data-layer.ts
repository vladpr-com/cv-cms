...
class CvCmsDatabase extends Dexie {
...
let dbInstance: CvCmsDatabase | null = null;

function getDb(): CvCmsDatabase {
  if (!dbInstance) {
    dbInstance = new CvCmsDatabase();
  }
  return dbInstance;
}

export class ClientDataLayer implements DataLayer {
  private db: CvCmsDatabase;
  ...
  async updateProfile(data: UpdateProfile): Promise<Profile> {
    const existing = await this.db.profile.get('default');
    const updated: DexieProfile = {
      id: 'default',
      ...
      website: data.website !== undefined ? data.website : (existing?.website ?? null),
      telegram: data.telegram !== undefined ? data.telegram : (existing?.telegram ?? null),
      updatedAt: new Date().toISOString(),
    };
   ...
   */
  async exportAllRawData(): Promise<{ jobs: Job[]; highlights: Highlight[]; profile?: Profile }> {
    const allJobs = await this.db.jobs.toArray();
    const allHighlights = await this.db.highlights.toArray();
    const profileData = await this.getProfile();
    return {
