...
const CONTACT_DISPLAY = [
  { key: 'email' as const, icon: Mail, href: (v: string) => `mailto:${v}`, display: (v: string) => v },
  { key: 'phone' as const, icon: Phone, href: (v: string) => `tel:${v}`, display: (v: string) => v },
  { key: 'location' as const, icon: MapPin, href: null, display: (v: string) => v },
  { key: 'linkedin' as const, icon: Linkedin, href: (v: string) => v.startsWith('http') ? v : `https://linkedin.com/in/${v}`, display: (v: string) => v.replace(/^https?:\/\/(www\.)?linkedin\.com\/in\//, '').replace(/\/$/, '') },
  { key: 'github' as const, icon: Github, href: (v: string) => v.startsWith('http') ? v : `https://github.com/${v}`, display: (v: string) => v.replace(/^https?:\/\/(www\.)?github\.com\//, '').replace(/\/$/, '') },
  { key: 'website' as const, icon: Globe, href: (v: string) => v.startsWith('http') ? v : `https://${v}`, display: (v: string) => v.replace(/^https?:\/\/(www\.)?/, '').replace(/\/$/, '') },
  { key: 'telegram' as const, icon: Send, href: (v: string) => `https://t.me/${v.replace('@', '')}`, display: (v: string) => v.startsWith('@') ? v : `@${v}` },
] as const;

function ContactsRow({ profile, onEditClick }: { profile: Profile; onEditClick: () => void }) {
  const filled = CONTACT_DISPLAY.filter((c) => profile[c.key]);
  if (filled.length === 0) {
    return (
      <button
        onClick={onEditClick}
        className="flex items-center gap-1 text-xs text-muted-foreground/60 hover:text-muted-foreground transition-colors mt-0.5"
      >
      ...
      {filled.map((c) => {
        ...
        const content = (
          <span className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors">
            ...
            <span>{c.display(value)}</span>
          </span>
        );
        if (c.href) {
          return (
            <a key={c.key} href={c.href(value)} target="_blank" rel="noopener noreferrer">
              {content}
            </a>
          );
        }
        return <span key={c.key}>{content}</span>;
      })}
      <button
        onClick={onEditClick}
...
function UnifiedFeedContent({
  ...
  useEffect(() => {
    ...
    async function loadFromIndexedDB() {
      try {
        ...
        const [jobsData, domainsData, skillsData, loadedProfile] = await Promise.all([
        ...
        if (loadedProfile) {
          ...
          if (loadedProfile.fullName) setFullName(loadedProfile.fullName);
        }
      } catch (error) {
  ...
  const handleContactsSave = useCallback(async (data: UpdateProfile) => {
    try {
      if (mode === 'anonymous' && dataLayerRef.current) {
        const updated = await dataLayerRef.current.updateProfile(data);
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          <div className="flex items-center justify-between py-6 border-b border-border/40">
            <div>
              ...
              <ContactsRow profile={profileData} onEditClick={() => setShowContactsEditor(true)} />
      ...
      <ContactsEditor
        ...
        trigger={<span className="hidden" />}
        open={showContactsEditor}
        onOpenChange={setShowContactsEditor}
      />
    </div>
  );
}
