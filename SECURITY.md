...
2. **Authentication**
   - CV CMS uses Auth.js v5 with GitHub and Google OAuth providers
   - Each authenticated user gets an isolated Turso database (multi-tenant)
   - Anonymous users store data only in their browser's IndexedDB
   - JWT sessions are used — no server-side session storage
